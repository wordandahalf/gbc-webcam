...
DEVICE_DESCRIPTION = 'STLINK-V3 - ST-Link VCP Ctrl'
BAUD = 2_000_000
...
class SensorStream(Packetizer):
    ...
    TERMINATOR = bytes([ 0xde, 0xad, 0xbe, 0xef ])
    viewer = None
...
class Viewer:
    ...
    def on_data(self, data):
        ...
        raw_frame = np.frombuffer(data[0:(len(data) // 2) * 2], dtype=np.uint16).astype(np.float32) / 256
        N = min(HEIGHT * WIDTH, len(raw_frame))

        frame = np.zeros((HEIGHT, WIDTH)).ravel()
        frame[0:N] = raw_frame[0:N]
        frame = np.roll(frame.reshape((HEIGHT, WIDTH)), shift=5, axis=1)
